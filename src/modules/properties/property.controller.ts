...
export const updateProperty = catchAsync(async (req: AuthRequest, res: Response) => {
  const landlordId = req.user!.userId;
  const property = await propertyService.updateProperty(String(req.params.id), landlordId, req.body);
  res.status(200).json({
    success: true,
...
export const deleteProperty = catchAsync(async (req: AuthRequest, res: Response) => {
  const landlordId = req.user!.userId;
  await propertyService.deleteProperty(String(req.params.id), landlordId);
  res.status(200).json({
    success: true,
